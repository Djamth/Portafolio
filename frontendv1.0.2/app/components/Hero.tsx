    ...
    <section id="home" className="relative isolate overflow-hidden bg-[#080d17] pb-12 pt-16 text-[#f5faff] lg:pb-16 lg:pt-24">
      ...
      <div className="section-shell relative z-10 grid items-center gap-10 lg:grid-cols-[1.03fr_.97fr]">
        ...
        <div className="relative mx-auto h-[460px] w-full max-w-[580px] sm:h-[540px] lg:h-[620px]">
          <HeroOrb />
          <div aria-hidden="true" className="dot-grid pointer-events-none absolute right-0 top-24 h-44 w-44 opacity-60" />
          <div className="pointer-events-none absolute inset-0 z-20"><Image src="/portfolio/DJ.png" alt="Denis Jamil Tineo Huancas, desarrollador backend" fill priority sizes="(min-width: 1024px) 580px, 90vw" className="hero-image-mask object-contain object-bottom drop-shadow-[0_18px_28px_rgba(0,0,0,0.35)]" /></div>
          <div aria-hidden="true" className="pointer-events-none absolute bottom-6 left-4 z-30 h-44 w-[105%] rounded-[50%] border-b-2 border-cyan-300/70 sm:h-56" />
          <div className="pointer-events-none absolute bottom-[45px] right-0 z-40 hidden w-60 rounded-2xl border border-cyan-300/20 bg-[#111c2e]/95 p-5 text-xs text-[#cbd8e8] shadow-2xl backdrop-blur sm:block"><p className="mb-3 font-bold text-[#00d9ff]">&lt;/&gt;</p><p><span className="text-[#18e0b5]">const</span> developer = {"{"}</p><p className="pl-3"><span className="text-[#79ebff]">role:</span> <span className="text-[#b8e986]">&quot;Backend&quot;</span>,</p><p className="pl-3"><span className="text-[#79ebff]">focus:</span> <span className="text-[#b8e986]">&quot;APIs limpias&quot;</span></p><p>{"};"}</p></div>
        </div>
      </div>
